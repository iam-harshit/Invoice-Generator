...
const mapStateToProps = (state) => {
  return {
    ...
    invoiceToEdit: state.invoice.invoiceToEdit,
  };
};
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setInvoiceToEdit: (invoice) => dispatch(setInvoiceToEdit(invoice)),
});

class InvoiceForm extends React.Component {
  constructor(props) {
    super(props);
    const invoiceToEdit = this.props.invoiceToEdit || {};
    this.state = {
      isOpen: false,
      currency: "$",
      currentDate: invoiceToEdit.currentDate || "",
      invoiceNumber: invoiceToEdit.invoiceNumber || 1,
      dateOfIssue: invoiceToEdit.dateOfIssue || "",
      billTo: invoiceToEdit.billTo || "",
      billToEmail: invoiceToEdit.billToEmail || "",
      billToAddress: invoiceToEdit.billToAddress || "",
      billFrom: invoiceToEdit.billFrom || "",
      billFromEmail: invoiceToEdit.billFromEmail || "",
      billFromAddress: invoiceToEdit.billFromAddress || "",
      notes: invoiceToEdit.notes || "",
      total: invoiceToEdit.total || "0.00",
      subTotal: invoiceToEdit.subTotal || "0.00",
      taxRate: invoiceToEdit.taxRate || "",
      taxAmmount: invoiceToEdit.taxAmmount || "0.00",
      discountRate: invoiceToEdit.discountRate || "",
      discountAmmount: invoiceToEdit.discountAmmount || "0.00",
  ...
  handleFormSubmit = (event) => {
    ...
    } else if (this.state.action === "save") {
      this.addInvoiceHandler();
    }
  };

  addInvoiceHandler = () => {
    if (this.props.invoiceToEdit) {
      const editedInvoice = { ...this.state };
      this.props.editInvoice(editedInvoice);
    } else {
    ...
    this.props.setInvoiceToEdit(null);
  };

  componentDidMount(prevProps) {
    this.handleCalculateTotal();
  }
  handleRowDel(items, event) {
          ...
          <Col md={4} lg={3}>
            <div className="sticky-top pt-md-3 pt-xl-4">
              <Button
                ...
                onClick={() => this.setState({ action: "review" })}
              >
                Review Invoice
              </Button>
              <Button
